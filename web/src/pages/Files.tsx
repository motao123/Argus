  ...
  const read = useMutation({
    ...
    onSuccess: (r) => {
      try {
        ...
        setPreview(`(二进制文件 ${fmtBytes(r.size)})`);
      }
    },
  });
    ...
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted">浏览远端服务器文件系统</p>
        </div>
        <select
          value={serverId}
          onChange={(e) => {
      ...
      <div className="mb-3 flex items-center gap-2 rounded-xl border border-border bg-panel p-3">
        <button onClick={up} className="rounded p-1.5 hover:bg-black/5 dark:hover:bg-white/5" title="上级目录">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <span className="flex-1 truncate font-mono text-sm">{path}</span>
        <button onClick={() => refetch()} className="rounded p-1.5 hover:bg-black/5 dark:hover:bg-white/5" title="刷新">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-xs text-muted">
              ...
              <th className="px-4 py-2.5 font-normal">大小</th>
              <th className="px-4 py-2.5 font-normal">权限</th>
              <th className="px-4 py-2.5 font-normal">修改时间</th>
          ...
          <tbody>
            {entries.map((e) => (
              ...
              >
                ...
                <td className="px-4 py-2.5 tabular text-muted">{new Date(e.modified * 1000).toLocaleString("zh-CN", { hour12: false })}</td>
                <td className="px-4 py-2.5">
                  <div className="flex justify-end gap-1">
                    ...
                    <button
                      onClick={(ev) => {
                        ev.stopPropagation();
                        if (confirm(`删除 ${e.is_dir ? "目录" : "文件"}「${e.name}」？`)) del.mutate(e);
                      }}
                      title="删除"
      ...
      {uploading && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/40">
          <div className="rounded-xl border border-border bg-panel p-5 text-sm">上传中…</div>
        </div>
      )}
    </div>
  );
