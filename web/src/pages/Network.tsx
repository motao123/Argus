...
const emptyDDNS: DDNSForm = { server_id: 0, name: "", provider: "webhook", record_type: "A", access_key: "", secret_id: "", secret_key: "", domains: "", webhook_url: "", webhook_method: "GET", webhook_headers: "{}", webhook_body: "", enabled: true };
      ...
      <section>
        ...
        {ddnsForm && (
          <div className="mb-4 grid grid-cols-1 gap-3 rounded-xl border border-border bg-panel p-4 md:grid-cols-2">
            ...
            <select value={ddnsForm.provider} onChange={(e) => setDDNSForm({ ...ddnsForm, provider: e.target.value as DDNSForm["provider"] })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm"><option value="webhook">Webhook</option><option value="cloudflare">Cloudflare</option><option value="tencent">Tencent DNSPod</option><option value="he">Hurricane Electric</option></select>
            ...
            </> : ddnsForm.provider === "tencent" ? <>
              <input type="password" aria-label={t("network.tencentSecretId")} placeholder={t("network.tencentSecretIdHint")} value={ddnsForm.secret_id} onChange={(e) => setDDNSForm({ ...ddnsForm, secret_id: e.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm" />
              <input type="password" aria-label={t("network.tencentSecretKey")} placeholder={t("network.tencentSecretKeyHint")} value={ddnsForm.secret_key} onChange={(e) => setDDNSForm({ ...ddnsForm, secret_key: e.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm" />
            </> : <input type="password" aria-label={ddnsForm.provider === "he" ? t("network.heKey") : t("network.cfToken")} placeholder={ddnsForm.provider === "he" ? t("network.heKeyHint") : t("network.cfTokenHint")} value={ddnsForm.access_key} onChange={(e) => setDDNSForm({ ...ddnsForm, access_key: e.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm md:col-span-2" />}
