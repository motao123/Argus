  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5">
        ...
        />
        <Row icon={<Network className="h-3.5 w-3.5" />} label="网络" value={`↓ ${fmtSpeed(server.net_in_speed)}  ↑ ${fmtSpeed(server.net_out_speed)}`} />
      </div>

      <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-xs text-muted">
        <span>负载 {server.load1.toFixed(2)}</span>
        <span>{fmtUptime(server.uptime)}</span>
      </div>
    </Link>
  );
}
