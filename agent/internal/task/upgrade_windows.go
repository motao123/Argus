...
func upgradeSelf(p *protocol.UpgradeParams) (string, error) {
	...
	if powershell, lookErr := exec.LookPath("powershell.exe"); lookErr == nil {
		script := fmt.Sprintf("Start-Sleep -Seconds 1; Move-Item -LiteralPath %s -Destination %s -Force; Start-Process -FilePath %s -ArgumentList %s", psQuote(newPath), psQuote(self), psQuote(self), psQuote(args))
		cmd = exec.Command(powershell, "-NoProfile", "-NonInteractive", "-Command", script)
	} else {
		scriptPath := self + ".upgrade.cmd"
		script := fmt.Sprintf("@echo off\r\nping 127.0.0.1 -n 2 >nul\r\nmove /Y \"%s\" \"%s\" >nul\r\nstart \"\" \"%s\" %s\r\ndel \"%%~f0\"\r\n", newPath, self, self, args)
