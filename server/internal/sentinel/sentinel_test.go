...
func TestRecordPersistsWindowQuantiles(t *testing.T) {
	...
	for i := 0; i < 15; i++ {
		s.record(2, protocol.ServiceCheckResult{Up: true, DelayMs: 0})
		s.record(2, protocol.ServiceCheckResult{Up: true, DelayMs: 100})
	}
	...
	s.record(2, protocol.ServiceCheckResult{Up: false, DelayMs: 0})
	got = model.ServiceHistory{}
	if err := db.Where("service_id = ?", 2).First(&got).Error; err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < DelayMinSamples; i++ {
		s.record(3, protocol.ServiceCheckResult{Up: true, DelayMs: 5})
	}
