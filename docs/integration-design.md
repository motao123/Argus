...
| 阶段 | 内容 | 状态 |
|---|---|---|
| A1-A6 | 统一响应壳/服务故障通知/运维子命令/站点设置/分组/批量操作/离线通知 | ✅ |
| B1-B6 | 服务监控/文件管理器/流量报告+到期提醒/可用率色块+图表拼接/WAF/配置下发+MCP fs/标签排序隐藏/消息发送测试 | ✅ |
| 第1-7项 | 私有站点模式/负载达标比例告警/JS 通知渠道/服务统计聚合/OAuth2 登录/pprof+分片上传恢复/XtermJS 外观设置 | ✅ |
| 2026-08-16 补充 | Agent 密钥读取链路修复（GET /users/:id/secret + 引导生成 + 存量回填）、在线会话管理页、站点设置页 | ✅ |
| 剩余候选 | 管理界面覆盖度（DDNS/NAT/插件/备份/审计/数据库工具 UI）、周期流量统计、报警联动触发任务、主题市场、i18n 全量 | 📋 |

> 架构说明：第 2 节 API 层路由与第 5 节页面规划均已按现状落地；页面清单以 web/src/main.tsx 为准。
